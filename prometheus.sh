...
cp -r /root/11.1.create_prometheus_server/alert-manager/alert-rules /opt/prometheus/ &>>$LOGFILE
VALIDATE $? "copied alert rules"

cp /root/11.1.create_prometheus_server/prometheus.yml prometheus/prometheus.yml &>>$LOGFILE
VALIDATE $? "copied prometheus configuration"

cp /root/11.1.create_prometheus_server/prometheus.service /etc/systemd/system/prometheus.service &>>$LOGFILE
